...
class index {
    ...
    public function upload(){
            ...
            $size=$_FILES["uploads"]['size']/1024;
            if($size>2000){
                alert::message(-1,'檔案限制2MB');
			}

            $tp = array("image/jpeg","image/jpg","application/pdf");    //检查上传文件是否在允许上传的类型

                if(!in_array($_FILES["uploads"]["type"],$tp)){
                    alert::message(-1,'文件类型错误');
